...
- (void)statusbarChange: (NSNotification *)notification {
    ...
    BOOL showNavi = [obj objectForKey:@"showNavi"];
    self.showNavi = showNavi;
    [self resizeWebView];
...
//    _statusBarBackgroundView.autoresizesSubviews = NO;
}
...
-(void)resizeWebView
{
    ...
    UINavigationController *naviVC = (UINavigationController *) self.viewController.parentViewController;
    
     CGRect screen = [self.viewController.view convertRect:bounds fromView:naviVC.view];

    self.viewController.view.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
    ...
    if (!self.statusBarOverlaysWebView) {
        if (screen.origin.y < 0  ) {
            // 判断是否显示navibar，高度44
                   if (self.showNavi) {
                      frame.origin.y = height + 44;
                   }else {
